...
const createAppointment = async (req, res) => {
  try {
    ...
    const conflictCheck = await pool.query(
      "SELECT id FROM appointments WHERE clinic_id = $1 AND doctor_id = $2 AND appointment_date = $3 AND status = 'scheduled'", [clinic_id, doctor_id, appointment_date]);
...
const updateAppointmentStatus = async (req, res) => {
  try {
    ...
    if (!validStatuses.includes(status)) {
      return res.status(400).json({ error: "حالة الميعاد غير صالحة (Invalid status)" });
    }
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ error: "الميعاد غير موجود في هذه العيادة" });
    }
